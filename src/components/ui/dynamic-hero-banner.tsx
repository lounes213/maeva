...
export function DynamicHeroBanner({
  ...
  useEffect(() => {
    const fetchLatestCollection = async () => {
      try {
        // Fetch featured collections first, sorted by most recent
        const res = await fetch('/api/collection?isFeatured=true&limit=1');
        const data = await res.json();
        
        if (data.data && data.data.length > 0) {
          setLatestCollection(data.data[0]);
        } else {
          // If no featured collections, fetch the most recent collection
          const fallbackRes = await fetch('/api/collection?limit=1');
          const fallbackData = await fallbackRes.json();
          
          if (fallbackData.data && fallbackData.data.length > 0) {
            setLatestCollection(fallbackData.data[0]);
          }
        }
      } catch (error) {
  ...
  const imageSrc = latestCollection && latestCollection.image && latestCollection.image.length > 0
    ? latestCollection.image[0]
    : fallbackImageSrc;

  // Determine the collection link
  const collectionLink = latestCollection 
    ? `/collection/${latestCollection._id}`
    : primaryButtonLink;
        ...
        >
          ...
          >
            <Link href={collectionLink}>
              ...
              >
                {latestCollection ? `Découvrir ${latestCollection.name}` : primaryButtonText}
              </ModernButton>
            </Link>
        ...
        >
          <div className="relative rounded-3xl overflow-hidden shadow-2xl border-8 border-white">
            {loading ? (
              ...
              <Image 
                src={imageSrc} 
                alt={latestCollection?.name || imageAlt} 
                width={600} 
                height={800} 
